...
public class JsNotInStaticResourceHtmlScript extends AbstractVfRule {
    ...
    protected @NonNull RuleTargetSelector buildTargetSelector() {
        return RuleTargetSelector.forTypes(ASTHtmlScript.class);
    }
    ...
    private void processASTAttribute(ASTHtmlScript node, Object data) {

        ASTAttribute srcAttr =
                node.children(ASTAttribute.class)
                .filterMatching(ASTAttribute::getName, "src").get(0);

        if (srcAttr == null) {
            return;
        }

        ASTAttributeValue srcAttrValue = srcAttr.children(ASTAttributeValue.class).get(0);
        
        int countOfAttrExpressions = srcAttr.descendants(ASTExpression.class).count();
        if (countOfAttrExpressions != 0) {
            handleExpression(srcAttrValue,data);
        } else {
            ASTText srcAttrText = srcAttrValue.children(ASTText.class).get(0);
            handleLiteralStringSrc(srcAttrText,data);
        }
    }
