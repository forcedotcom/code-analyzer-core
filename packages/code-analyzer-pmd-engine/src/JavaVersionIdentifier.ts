...
export class RuntimeJavaVersionIdentifier implements JavaVersionIdentifier {
    ...
    identifyJavaVersion(javaCommand: string): Promise<SemVer|null> {
        ...
        return new Promise<SemVer|null>((resolve, reject) => {
            ...
            // never the inherited, attacker-controllable scanned-repo cwd. This identifier is shared by both
            // the PMD and CPD sub-engines, so this single pin covers both. On Windows, spawning a bare command
            // name like "java" (the auto-detect fallback) without pinning cwd lets the OS resolve a repo-local
            // java/java.exe from the current directory ahead of the trusted PATH entry, executing attacker code
            // (CWE-427 search-path element). Absolute-path and PATH-resolved java commands are unaffected, so
            // behavior is preserved for legitimate configs. Mirrors the Flow engine's PythonCommandExecutor
            // cwd pin.
