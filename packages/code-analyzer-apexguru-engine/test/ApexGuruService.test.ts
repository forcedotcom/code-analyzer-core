...
describe('ApexGuruService', () => {
    ...
    beforeEach(() => {
        ...
        (Connection.create as jest.Mock) = jest.fn().mockResolvedValue(mockConnection);

        // Mock AuthInfo.create
        (AuthInfo.create as jest.Mock) = jest.fn().mockResolvedValue({});
    ...
    describe('initialize', () => {
        it('should throw error when hardcoded credentials not set', async () => {
            ...
            await expect(apexGuruService.initialize())
    ...
    describe('validate', () => {
        beforeEach(() => {
            // Manually inject mock connection to bypass initialize()
            (apexGuruService as any).connection = mockConnection;
        });
