...
describe('Tests for the ESLint8EnginePlugin', () => {
    let plugin: EnginePluginV1;
    beforeAll(() => {
        plugin = new ESLint8EnginePlugin();
    });
    ...
    it('When createEngineConfigDescription is with a valid engine name, then return the correct ConfigDescription', async () => {
        expect(plugin.describeEngineConfig(ESLint8Engine.NAME)).toEqual(ESLINT_ENGINE_CONFIG_DESCRIPTION);
    });
    ...
    it('When createEngine is passed eslint and a valid config, then an ESLint8Engine instance is returned', async () => {
        const engine: Engine = await plugin.createEngine('eslint', DEFAULT_CONFIG);
        expect(engine).toBeInstanceOf(ESLint8Engine);
    });
});
...
async function callCreateEngineConfig(plugin: EnginePluginV1, userProvidedOverrides: ConfigObject, configRoot?: string): Promise<ConfigObject> {
    const configValueExtractor: ConfigValueExtractor = new ConfigValueExtractor(userProvidedOverrides, `engines.${ESLint8Engine.NAME}`, configRoot);
    return await plugin.createEngineConfig(ESLint8Engine.NAME, configValueExtractor);
}
